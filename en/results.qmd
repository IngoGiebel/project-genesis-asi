...
for answer in AI_CONSCIOUSNESS_COLORS["domain"]:          # fixed order
    rows = survey_submissions_df.query(
        "ai_can_be_conscious == @answer"
    )
    if rows[ [c for c, _ in FREE_TEXT_FIELDS] ].isna().all(axis=None):
        continue

    md_lines.append(f"## {AI_CONSCIOUSNESS_LABELS_PY[answer]}\n")
    ...
    for col, title in FREE_TEXT_FIELDS:
        ...
        # block-quote each response
        md_lines.extend(f"> {txt}\n" for txt in subset)
        md_lines.append("")  # blank line after each subsection
